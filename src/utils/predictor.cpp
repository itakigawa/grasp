...
#define USAGE " features atom.dict graphs sequences train.table"
...
int main(int argc, char **argv)
{
  ...
  string filetable(argv[optind + 4]);
  ...
  std::cout << "Predictor version 0.1" << std::endl;
  ...
  for (map<int, set<int> >::iterator sitr = ssave.begin(); sitr != ssave.end(); ++sitr)
  {
    for (map<int, set<int> >::iterator gitr = gsave.begin(); gitr != gsave.end(); ++gitr)
    {
      int id_s = sitr->first;
      int id_g = gitr->first;
      set<int> &tmpset_s = sitr->second;
      set<int> &tmpset_g = gitr->second;
