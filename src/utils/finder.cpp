...
#define USAGE " features atom.dict graphs sequences pairs"
...
int main(int argc, char **argv)
{
  ...
  if (filepair.substr(filepair.size() - 3, 3) == ".gz")
  {
    ...
    fs.push(gzip_decompressor());
    fs.push(pair_file);
    classify = f.read_pairs(fs);
  }
  else
  {
    classify = f.read_pairs(pair_file);
  }
  ...
  string filename = outdir + "/feat.table";
  std::ofstream o_file(filename.c_str());

  int count = 1;
  int numall = f.trans.size();
  for (vector<datapack>::iterator itr = f.trans.begin(); itr != f.trans.end(); ++itr)
  {
    ...
    int perc = numall / 100;
    if (count % perc == 0)
      std::cout << (1000 * count / numall) / 10.0 << "% " << f.s_tname[id_s] << "<->" << f.g_tname[id_g] << std::endl;
    count++;
    ...
    for (set<int>::iterator it1 = tmpset_s.begin(); it1 != tmpset_s.end(); ++it1)
    {
      ...
      for (set<int>::iterator it2 = tmpset_g.begin(); it2 != tmpset_g.end(); ++it2)
      {
        ...
        {
          row.insert(mapper2->second);
        }
      }
    }

    bool start = true;
    for (set<int>::iterator it3 = row.begin(); it3 != row.end(); ++it3)
    {
      if (start)
      {
        start = false;
